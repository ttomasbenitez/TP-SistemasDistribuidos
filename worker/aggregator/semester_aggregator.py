...
class SemesterAggregator(Worker):
    ...
    def _send_grouped_item(self, message, item):
        ...
        self.data_output_queue.send(new_message.serialize())
...
def main():
    ...
    aggregator = SemesterAggregator(data_input_queue, data_output_queue, eof_exchange, config_params["rabbitmq_host"], config_params["eof_service_queue"],  config_params["eof_self_queue"])
    aggregator.start()
