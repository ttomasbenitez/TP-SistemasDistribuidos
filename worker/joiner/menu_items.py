...
class QuantityAndProfit(Worker):
    ...
    def __init__(self, menu_items_in_queue: MessageMiddlewareQueue, products_in_queue: MessageMiddlewareQueue, out_queue: MessageMiddlewareQueue):
        super().__init__(products_in_queue)
        self.out_queue = out_queue
        self.menu_items_in_queue = menu_items_in_queue
    ...
    def close(self):
        try:
            self.in_queue.close()
            self.menu_items_in_queue.close()
...
def initialize_config():
    ...
    if config_params["rabbitmq_host"] is None or config_params["input_queue_1"] is None or config_params["input_queue_2"] is None or config_params["output_queue"] is None:
...
def main():
    ...
    aggregator = QuantityAndProfit(menu_items_input_queue, data_input_queue, output_queue)
    aggregator.start()
